...
static XMP_Node* FindExistingNode(XMP_Node* xmpParent, const XML_Node& xmlNode, bool isTopLevel)
{
	if (xmlNode.ns.empty())
...
//	XMP_OptionBits childOptions = 0;
	...
	if (isTopLevel)
	{
		...
		if (schemaNode->options & kXMP_NewImplicitNode)
			schemaNode->options ^= kXMP_NewImplicitNode;	// Clear the implicit node bit.
	...
	if (!(isArrayItem | isValueNode))
		if (auto node= FindChildNode(xmpParent, childName, kXMP_ExistingOnly))
			return node;
...
{
	// ignore duplicates
	if (FindExistingNode(xmpParent, xmlNode, isTopLevel))
		return;
